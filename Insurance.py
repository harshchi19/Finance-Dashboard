...
def insurance():
    # Sidebar title
    st.sidebar.title('Insurance Navigation')
    with st.sidebar:
    # Create the option menu in the sidebar
        selected = option_menu(
            menu_title="Insurance Menu",
            options=[
                'Choose From Base Plans', 
                'Make Your Own Plans', 
                'Fitness Score'
            ],
            icons=[
                'check-square',  # Icon for Choose From Base Plans
                'pencil',        # Icon for Make Your Own Plans
                'activity'       # Icon for Fitness Score
            ],
            menu_icon="cast",  # Main icon for the menu
            default_index=0,   # Default selected option
            orientation="vertical"  # Ensure the menu is vertical
        )
    
    # Display content based on selected option
    if selected == "Choose From Base Plans":
        choose_base_plans_page()
    elif selected == "Make Your Own Plans":
        make_your_own_plan_page()
    elif selected == "Fitness Score":
        fitness_score_page()
